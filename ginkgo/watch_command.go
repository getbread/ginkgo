...
import (
	...
	"github.com/getbread/ginkgo/ginkgo/watch"
)
...
func (w *SpecWatcher) WatchSuites(args []string, additionalArgs []string) {
	...
	if len(suites) == 1 {
		...
		w.suiteRunner.RunSuites(runners, w.commandFlags.NumCompilers, true, w.commandFlags.WithRetries, nil)
		runners[0].CleanUp()
	}
	...
	for {
		...
		case <-ticker.C:
			...
			if len(suitesToRun) > 0 {
				...
				result, _ := w.suiteRunner.RunSuites(runners, w.commandFlags.NumCompilers, true, w.commandFlags.WithRetries, func(suite testsuite.TestSuite) {
